...
function void router_dest_agt_top::build_phase(uvm_phase phase);
  ...
  if(!uvm_config_db #(router_env_config)::get(this, "", "router_env_config", m_cfg))
	...
	agenth = new[m_cfg.number_of_dest];
  foreach (agenth[i]) begin
    agenth[i] = router_dest_agent::type_id::create($sformatf("agenth[%0d]", i), this);
    uvm_config_db#(dest_agent_config)::set(this, $sformatf("agenth[%0d]*", i),
                                             "dest_agent_config", m_cfg.m_dest_agent_cfg[i]);
  end
endfunction

task router_dest_agt_top::run_phase(uvm_phase phase);
